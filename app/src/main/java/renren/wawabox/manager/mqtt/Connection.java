...
public class Connection {
    ...
    public static Connection createConnection(String clientHandle, String clientId, String host, int port, Context context, boolean tlsConnection) {
        ...
        LogUtil.d("kkkkkkkk", "clientHandle --> " + clientHandle + " clientId --> " + clientId + " host --> " + host + " port --> " + port);
     ...
     */
    public void addAction(String action, Throwable e) {
        ...
        if (e == null) {
            LogUtil.e(Constants.TAG, "addAction action --> " + action, LogUtil.LOG_FILE_NAME_MQTT);
        } else {
            LogUtil.e(Constants.TAG, "addAction action --> " + action + " e --> " + e.getMessage(), LogUtil.LOG_FILE_NAME_MQTT);
        }
...
//        notifyListeners(new PropertyChangeEvent(this, ActivityConstants.historyProperty, null, null));
    ...
    public void changeConnectionStatus(ConnectionStatus connectionStatus) {
        ConnectionStatus old = status;
        status = connectionStatus;
        notifyListeners((new PropertyChangeEvent(this, MqttConstants.ConnectionStatusProperty, old, status)));
    }
    ...
    private void notifyListeners(PropertyChangeEvent propertyChangeEvent) {
        for (int i = 0; i < listeners.size(); i++) {
            PropertyChangeListener listener = listeners.get(i);
            if (listener != null) {
    ...
    public void addNewSubscription(Subscription subscription) throws MqttException {
        addNewSubscription(subscription, null);
    }

    public void addNewSubscription(Subscription subscription, IMqttActionListener listener) throws MqttException {
        if (!subscriptions.containsKey(subscription.getTopic())) {
            ...
            final ActionListener callback = new ActionListener(this.context,
                    ActionListener.Action.SUBSCRIBE, this, listener, actionArgs);
...
//        MqttKeeper.log("disconnect " + this);
        LogUtil.d(Constants.TAG, "disconnect", LogUtil.LOG_FILE_NAME_MQTT);
        if (this.getClient() != null) {
            try {
                getClient().disconnect();
            } catch (MqttException e) {
//                MqttKeeper.error("disconnect error", e);
                LogUtil.e(Constants.TAG, "disconnect error \n" + e.getMessage(), LogUtil.LOG_FILE_NAME_MQTT);
            } catch (NullPointerException e) {
//                MqttKeeper.error("service null", e);
                LogUtil.e(Constants.TAG, "service null \n" + e.getMessage(), LogUtil.LOG_FILE_NAME_MQTT);
            }
        }
    }
...
//        MqttKeeper.log(msg);
        LogUtil.d(MqttManager.TAG, msg, LogUtil.LOG_FILE_NAME_MQTT);
